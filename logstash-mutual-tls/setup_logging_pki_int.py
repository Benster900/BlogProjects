...
def create_http_session(general_config):
    ...
    # Generate session
    if general_config['client_cert_file_path'] != '' and general_config['ca_cert_file_path'] != '' and general_config['client_key_file_path'] != '' :
        session.cert = ( general_config['client_cert_file_path'], general_config['client_key_file_path'] )
...
def check_vault_token(session, vault_addr):
    ...
    with open( expanduser("~/.vault-token"), 'r') as f:
        ...
        if r.json().get('errors'):
            print ( Fore.RED +  f"[-] - {datetime.now()} - Vault token invlaid" + Style.RESET_ALL )
            sys.exit(1)
    return vault_token
...
def check_vault_mount_exists(vault_addr, vault_path):
    ...
    if (vault_path[:-1] != "/" and vault_path + "/" in r.json().keys()) or vault_path in r.json().keys():
        return True
    return False
...
def generate_vault_pki_int(vault_addr, pki_int, session):
    ...
    if check_vault_mount_exists(vault_addr, pki_int['vault_mount_path']) == False:
        ...
        print ( Fore.GREEN +  f"[+] - {datetime.now()} - Created PKI path: {pki_int['vault_mount_path']}" + Style.RESET_ALL )
    else:
        print(Fore.YELLOW + f"[*] - {datetime.now()} - SKIPPING creating mount for: {pki_int['vault_mount_path']}" + Style.RESET_ALL) 
        

    ################################################ Create certificate ################################################
    if check_vault_cert_exists(vault_addr, pki_int['vault_mount_path'], session) == False:
        ...
        csr_data = str()
        with open(f"{pki_int['output_dir_path']}/{pki_int['vault_mount_path']}.csr", 'w') as f:
            csr_data = result['data']['csr']
            f.write( str(csr_data) )
        os.remove(f"{pki_int['output_dir_path']}/{pki_int['vault_mount_path']}.csr")
        print ( Fore.GREEN + f"[+] - {datetime.now()} - Generated certificate signing request for: {pki_int['common_name']}" + Style.RESET_ALL )
        ...
        cert_data = str()
        with open(f"{pki_int['output_dir_path']}/{pki_int['vault_mount_path']}.crt", 'w') as f:
            cert_data = result['data']['certificate']
            f.write( cert_data )
        print ( Fore.GREEN + f"[+] - {datetime.now()} - Generated certificate for: {pki_int['common_name']}" + Style.RESET_ALL )
        ...
        print ( Fore.GREEN + f"[+] - {datetime.now()} - Write certificate to Vault for: {pki_int['common_name']}" + Style.RESET_ALL )
    else:
        print(Fore.YELLOW + f"[*] - {datetime.now()} - SKIPPING creating certificate for: {pki_int['common_name']}" + Style.RESET_ALL) 

    ################################################ Create role ################################################
    if check_vault_role_exists(vault_addr, pki_int['vault_mount_path'], pki_int['vault_role']) == False:
        #### Create role ####
        session.post(
        ...
        print ( Fore.GREEN + f"[+] - {datetime.now()} - Create role for: {pki_int['common_name']}" + Style.RESET_ALL )
    else:
        print(Fore.YELLOW + f"[*] - {datetime.now()} - SKIPPING creating role for: {pki_int['common_name']}" + Style.RESET_ALL) 
    
def generate_vault_leaf_cert(vault_addr, cert_leaf, logging_domain, leaf_cert_hostname, session):
    """
    ...
    print ( Fore.GREEN + f"[+] - {datetime.now()} - Generated private key and public certificate for: { cert_leaf['common_name'] }" + Style.RESET_ALL )

    #### Extract private key ####
    with open(f"{cert_leaf['output_dir_path']}/{  cert_leaf['common_name'].replace('.','-') }.key", 'w') as f:
        f.write( result['data']['private_key'] )

    #### Extract public cert and chain chain ####
    with open(f"{cert_leaf['output_dir_path']}/{ cert_leaf['common_name'].replace('.','-') }.crt", 'w') as f:
...
if __name__ == "__main__":
    ...
    my_parser.add_argument('--vault_addr', type=str, required=True, help="Example: https://vault.example:443")
    my_parser.add_argument('--logging_domain', type=str, required=True, help="Example: example.com")
    my_parser.add_argument('--config', type=str, default="conf/vault/vault_setup.yaml", help="Example: conf/vault/vault_setup.yaml")
    ...
    leaf_cert_group.add_argument('--server_leaf_cert_hostname', type=str, help="Example: logstash01")
    leaf_cert_group.add_argument('--client_leaf_cert_hostname', type=str, help="Example: filebeat01")
    ...
    if args.setup_logging_pki:
        generate_vault_pki_int(args.vault_addr, yaml_config['pki_int_dict']['loging_int'], session)         # Create logging PKI int
        generate_vault_pki_int(args.vault_addr, yaml_config['pki_int_dict']['server_logging_int'], session) # Create server logging PKI int
        generate_vault_pki_int(args.vault_addr, yaml_config['pki_int_dict']['client_logging_int'], session) # Create client logging PKI int
    elif args.create_leaf_cert is not None and args.server_leaf_cert_hostname is not None:
        # Create Logstash leaf cert
        generate_vault_leaf_cert(args.vault_addr, yaml_config['cert_leaf_dict']['servers'], args.logging_domain, args.server_leaf_cert_hostname, session)
    elif args.create_leaf_cert is not None and args.client_leaf_cert_hostname is not None:
        # Create Logstash leaf cert
        generate_vault_leaf_cert(args.vault_addr, yaml_config['cert_leaf_dict']['clients'], args.logging_domain, args.client_leaf_cert_hostname, session)
    else:
