...
def delivery_report(err, msg):
    ...
    if err is not None:
        ...
        print('[-] {0} - Message delivered to {1} [{2}]'.format(
            datetime.now(),
            msg.topic(), 
            msg.partition()))
...
def vti_enrich_url(vti_api_key, data):
    vti_url = 'https://www.virustotal.com/vtapi/v2/url/report?apikey={0}&resource={1}'.format(vti_api_key, data['columns']['url'])
...
if __name__ == "__main__":
    ...
    print ( "[+] {0} - kafka_osquery_raw_topic: {1}".format(datetime.now(), kafka_osquery_raw_topic) )
    print ( "[+] {0} - kafka_osquery_enriched_topic: {1}".format(datetime.now(), kafka_osquery_enriched_topic) )
    ...
    # Producer connector
    kafka_producer = Producer( {'bootstrap.servers': '{0}:{1}'.format( kafka_server, kafka_port )})
