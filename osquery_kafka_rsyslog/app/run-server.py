...
def slack_alert(cfg, file_sha256_hash, hostname, file_path, report):
    ...
    msg = "[+] Malicious file: {0} - {1} - {2} - `{3}` ".format(file_path, hostname, file_sha256_hash, report.json()['permalink'])
...
def main():
    ...
    for message in consumer:
        ...
        print ( "[*] - {0} Scanning file: {1} - {2} - {3}".format(datetime.now(), file_path, hostname, file_sha256_hash) )

        # Query Virustotal
        query_bool, report = query_virustotal(file_sha256_hash, cfg)
        if query_bool:
            print( "[+] - {0} Malicious file: {1} - {2} - {3}".format(datetime.now(), file_path, hostname, file_sha256_hash) )
