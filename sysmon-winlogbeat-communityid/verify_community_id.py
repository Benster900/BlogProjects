...
def generate_community_id(src_ip, src_port, dest_ip, dest_port, protocol) -> str():
    """
...
if __name__ == "__main__":
    ...
    with open(args.file, 'r') as csv_file:
        ...
        for row in csv_reader:
            ...
            gen_cid = generate_community_id(src_ip, src_port, dest_ip, dest_port, protocol)

            if gen_cid != sysmon_cid:
                print ( f"[-] - Source IP: {src_ip} Source port: {src_port} Destination IP: {dest_ip} Destination port: {dest_port} Protocol: {protocol} Sysmon CID: {sysmon_cid} Community ID: {gen_cid}" )
            else:
                print ( f"[+] - Source IP: {src_ip} Source port: {src_port} Destination IP: {dest_ip} Destination port: {dest_port} Protocol: {protocol} Sysmon CID: {sysmon_cid} Community ID: {gen_cid}" )
